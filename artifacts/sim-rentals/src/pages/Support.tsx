...
async function replyToTicket({ id, message }: { id: string; message: string }): Promise<Ticket> {
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border ${cfg.color}`}>
      {cfg.pulse ? <span className="h-1.5 w-1.5 rounded-full status-pulse bg-current" /> : <Icon className="h-3 w-3" />}
      {cfg.label}
    </span>
  );
}
...
function TicketCard({ ticket }: { ticket: Ticket }) {
  ...
  const replyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setReply("");
      toast({ title: "Reply sent", description: "Support can see your latest message." });
    },
      ...
      <button className="w-full text-left px-5 py-4 flex items-start gap-4" onClick={() => setOpen((v) => !v)}>
        ...
        <div className="flex-1 min-w-0 text-left">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-[10px] font-medium text-slate-500 bg-white/[0.04] border border-white/[0.07] px-2 py-0.5 rounded-full">{ticket.category}</span>
          </div>
        ...
        <div className="faq-inner">
          <div className="px-5 pb-5 space-y-4 border-t border-white/[0.05] pt-4">
            <div className="space-y-3">
              {(ticket.messages?.length ? ticket.messages : [{ id: `${ticket.id}-initial`, senderRole: "user", senderName: "You", message: ticket.message, createdAt: ticket.createdAt }]).map((message) => {
                ...
                return (
                  <div key={message.id} className={`rounded-xl p-4 border ${fromAdmin ? "bg-sky-400/[0.06] border-sky-400/[0.15]" : "bg-white/[0.02] border-white/[0.06]"}`}>
                    <p className={`text-[11px] font-semibold uppercase tracking-wider mb-1.5 flex items-center gap-1.5 ${fromAdmin ? "text-sky-400" : "text-slate-500"}`}>
                      {fromAdmin && <Zap className="h-3 w-3" />}
                      {fromAdmin ? "SKY SMS Support" : "You"} · {format(new Date(message.createdAt), "MMM d, HH:mm")}
                    </p>
                    <p className="text-sm text-slate-200 leading-relaxed whitespace-pre-wrap">{message.message}</p>
                  </div>
                );
              })}
            </div>
            ...
            {canReply ? (
              ...
              <p className="text-[12px] text-slate-500 italic">This ticket is {ticket.status}. Open a new ticket if you still need help.</p>
            )}
          </div>
        </div>
      </div>
...
export default function Support() {
  ...
  const { data: tickets = [], isLoading } = useQuery({ queryKey: ["support-tickets"], queryFn: fetchTickets, refetchInterval: 20_000 });
  ...
  return (
    <div className="space-y-7 max-w-2xl mx-auto">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="page-enter page-enter-d2 glass-card rounded-2xl p-6 space-y-5">
          <div className="flex items-center gap-3 mb-1">
            <div className="h-9 w-9 rounded-xl bg-sky-400/10 border border-sky-400/15 flex items-center justify-center"><LifeBuoy className="h-4 w-4 text-sky-400" /></div>
            <div><h2 className="text-[14px] font-bold text-white">Open a support ticket</h2><p className="text-[11px] text-slate-500">Fill in the details below and we'll get back to you.</p></div>
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Subject</label>
            <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="e.g. Balance not updated after payment" maxLength={200} required className="field-input w-full h-11 rounded-xl px-4 text-sm text-white placeholder:text-slate-600" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1"><Tag className="h-3 w-3" /> Category</label>
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="field-input w-full h-11 rounded-xl px-3 text-sm text-white cursor-pointer appearance-none">{CATEGORIES.map((c) => <option key={c} value={c} className="bg-[#0e1628]">{c}</option>)}</select>
            </div>
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1"><AlertCircle className="h-3 w-3" /> Priority</label>
              <select value={priority} onChange={(e) => setPriority(e.target.value)} className="field-input w-full h-11 rounded-xl px-3 text-sm text-white cursor-pointer appearance-none">{PRIORITIES.map((p) => <option key={p.value} value={p.value} className="bg-[#0e1628]">{p.label}</option>)}</select>
            </div>
          </div>
          <div className="space-y-1.5">
      ...
      {!showForm && tickets.length === 0 && !isLoading && (
        <div className="page-enter page-enter-d2 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[{ icon: Clock, title: "Response time", desc: "We reply within 24 hours on business days." }, { icon: MessageSquare, title: "Message thread", desc: "Reply to admin messages while your ticket is open." }, { icon: CheckCircle2, title: "Refund policy", desc: "Rental credits are refunded automatically if no SMS is received." }].map((item) => <div key={item.title} className="glass-card rounded-2xl p-5"><item.icon className="h-5 w-5 text-sky-400 mb-3" /><p className="text-[13px] font-semibold text-white mb-1">{item.title}</p><p className="text-[12px] text-slate-400 leading-relaxed">{item.desc}</p></div>)}
        </div>
      )}
