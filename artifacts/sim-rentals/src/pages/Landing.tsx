...
const features = [
  {
    ...
    desc: "Numbers are allocated in seconds. Receive SMS codes as soon as they arrive — no queues, no waiting.",
  },
  {
    icon: Globe,
    title: "Global Coverage",
    desc: "Browse 8+ countries with live availability counts. See real stock before spending a single credit.",
  },
  {
    icon: Lock,
    title: "Crypto Payments",
    desc: "Top up with BTC, ETH, USDT and 30+ coins via OxaPay. Fully private, borderless, no chargebacks.",
  },
  {
    icon: MessageSquare,
    title: "Live SMS Inbox",
    desc: "Incoming verification codes appear instantly on your rental card. One tap to copy the code.",
  },
  {
    icon: RefreshCw,
    title: "Cancel & Refund",
    desc: "Cancel any rental before the 20-minute window ends. Credits return to your balance instantly.",
  },
  {
    icon: Clock,
    title: "20-Min Window",
    desc: "A live countdown tracks your activation window. If no SMS arrives, you get a full automatic refund.",
  },
];
...
const faqs = [
  {
    ...
    a: "Buy credits, select a country and service, and get a temporary phone number instantly. Any incoming SMS is shown in your dashboard in real time. Numbers stay active for 20 minutes.",
  },
  {
  ...
  },
  {
    ...
    a: "We accept cryptocurrency payments via OxaPay, which supports BTC, ETH, USDT, and many other coins. Purchases are private and borderless.",
  },
  {
    q: "What if I don't receive an SMS?",
    a: "You can cancel an active rental before the 20-minute window closes and receive a full credit refund instantly. If the window expires with no SMS, you're also refunded automatically.",
  },
  {
    q: "Are the numbers reused?",
    a: "Numbers are recycled between sessions but each rental starts completely fresh — you only see messages received during your active window. No shared history.",
  },
];
    ...
    <div className="min-h-screen premium-shell overflow-x-hidden text-white">
      ...
      <header className="sticky top-4 z-50 mx-auto flex max-w-6xl justify-center px-4">
        <div className="glass-card flex h-14 w-full items-center justify-between rounded-full px-5 neon-border">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onLogin}
              className="hidden text-sm font-bold text-slate-400 transition hover:text-white sm:inline"
      ...
      <main className="relative z-10">
        ...
        <section id="home" className="relative mx-auto max-w-7xl px-4 pb-24 pt-28 text-center sm:px-6 lg:px-8 dot-grid">
          {/* Badge */}
          <div className="mx-auto mb-8 inline-flex items-center gap-2 rounded-full border border-cyan-400/25 bg-cyan-400/8 px-5 py-2 text-xs font-bold uppercase tracking-[0.22em] text-cyan-300">
          ...
          <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
            ...
            <a
              href="#features"
              className="rounded-full border border-white/15 px-8 py-4 text-sm font-bold text-white transition hover:border-cyan-400/40 hover:bg-cyan-400/8"
            >
              View Features
            </a>
          </div>
          ...
          <div className="marquee-track flex w-[200%] gap-12 whitespace-nowrap text-[clamp(2.5rem,5vw,4.5rem)] font-black uppercase tracking-tight text-white/[0.06]">
            {Array.from({ length: 10 }).map((_, i) => (
              <span key={i} className="shrink-0">
                SMS RENTALS &nbsp;<span className="text-cyan-400/20">✦</span>&nbsp;
              </span>
            ))}
          </div>
          {/* Second layer for visual depth */}
          <div className="absolute inset-0 flex items-center pointer-events-none">
            <div className="marquee-track flex w-[200%] gap-12 whitespace-nowrap text-[clamp(1.2rem,2vw,1.8rem)] font-bold uppercase tracking-[0.3em] text-white/[0.04]" style={{ animationDirection: "reverse", animationDuration: "40s", marginTop: "0.25rem" }}>
        ...
        <section id="services" className="py-24">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-14 text-center">
              <div className="mx-auto mb-5 w-fit rounded-full border border-cyan-400/20 bg-cyan-400/8 px-4 py-2 text-xs font-bold uppercase tracking-[0.22em] text-cyan-300">
                Supported Services
              </div>
              <h2 className="mb-4 text-[clamp(2rem,4vw,3.5rem)] font-black tracking-tight text-white">
                Every major platform
              </h2>
              <p className="mx-auto max-w-xl text-slate-400">
                Rent numbers for all the services you need — new providers added regularly.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {services.map((service) => (
                <div
                  key={service.name}
                  className="glass-card rounded-2xl p-5 flex items-center gap-4 cursor-default hover:scale-[1.02] transition-all duration-200"
                >
            ...
            <div className="mt-10 text-center">
              <Button
                onClick={onLogin}
                variant="outline"
                className="rounded-full border-cyan-400/25 text-cyan-300 hover:bg-cyan-400/10 hover:border-cyan-400/40 px-8"
              >
                Start renting →
              </Button>
            </div>
        ...
        <section id="features" className="py-24 relative">
          ...
          <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-14 text-center">
              ...
              <p className="mx-auto max-w-xl text-slate-400">
                Everything you need to rent numbers, receive codes, and manage credits — without clutter.
              </p>
            </div>

            <div className="grid gap-5 md:grid-cols-3">
              {features.map((feature) => (
                <div key={feature.title} className="glass-card rounded-3xl p-7 group hover:scale-[1.01] transition-all duration-200" data-testid={`card-feature-${feature.title.toLowerCase().replace(/\s+/g, "-")}`}>
              ...
              {faqs.map((faq, i) => (
                <div key={i} className="glass-card rounded-2xl overflow-hidden">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`h-4 w-4 shrink-0 text-cyan-400 transition-transform duration-200 ${openFaq === i ? "rotate-180" : ""}`}
                    />
                  </button>
                  {openFaq === i && (
